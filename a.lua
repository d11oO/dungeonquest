...
local HttpService = game:GetService("HttpService")
		...
		if queueonteleport then
			queueonteleport("loadstring(game:HttpGet('https://raw.githubusercontent.com/d11oO/dungeonquest/refs/heads/main/a.lua'))()")
		end
...
noclip() -- to toggle noclip() and clip()local TweenService = game:GetService("TweenService")
...
--Farming Start
local NormalFarmToggle = NormalFarm:AddToggle("NormalFarmToggle",{Text = "Enabled",Default = true,Risky = false})
NormalFarmToggle:OnChanged(function(value)
    Settings.AutoFarm.Enabled = value
end)
local UseSkillsToggle = NormalFarm:AddToggle("UseSkillsToggle",{Text = "Use Skills",Default = true,Risky = false})
...
local AutoRetryToggle = SettingsGroup:AddToggle("AutoRetryToggle",{Text = "Auto Retry",Default = true,Risky = false})
    ...
    while true do task.wait()
        if Character and Character:FindFirstChild("Head") and Character.Head:FindFirstChild("playerNameplate") and Players.LocalPlayer and Players.LocalPlayer.PlayerGui and Players.LocalPlayer.PlayerGui:FindFirstChild("HUD") and Players.LocalPlayer.PlayerGui.HUD:FindFirstChild("Main") and Players.LocalPlayer.PlayerGui.HUD.Main:FindFirstChild("PlayerStatus") and Players.LocalPlayer.PlayerGui.HUD.Main.PlayerStatus:FindFirstChild("PlayerStatus") and Players.LocalPlayer.PlayerGui.HUD.Main.PlayerStatus.PlayerStatus:FindFirstChild("PlayerName") then
            if Settings.Misc.NameHide == true then
                ...
                Character.Head.playerNameplate.PlayerName.Text = "Nil"
                Character.Head.playerNameplate.Title.Text="Nil"
                PlayerGui.PartyUi.Frame.CreateScreen.DungeonInfo.Owner.Text = "Nil"
                PlayerGui.PartyUi.Frame.CreateScreen.DungeonInfo.PartyName.Text = " Nil Party"
            end
        end
    end    
end)
            ...
            if Players.LocalPlayer and Players.LocalPlayer.Character and Players.LocalPlayer.Character:FindFirstChild("Humanoid") then
                ...
                if humanoid.Health > 0 then -- Check if the local player is alive
                    if Players.LocalPlayer.Backpack:FindFirstChild("Demonic Curse") then
                        ...
                        if blasts and blasts:FindFirstChild("abilityEvent") then
